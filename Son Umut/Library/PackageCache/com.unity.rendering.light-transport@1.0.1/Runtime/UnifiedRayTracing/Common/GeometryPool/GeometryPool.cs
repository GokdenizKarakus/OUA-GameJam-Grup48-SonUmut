...
namespace UnityEngine.Rendering.UnifiedRayTracing
{
    ...
    internal class GeometryPool : IDisposable
    {
        ...
        private const GraphicsBuffer.Target ComputeBufferTarget = GraphicsBuffer.Target.CopyDestination | GraphicsBuffer.Target.CopySource | GraphicsBuffer.Target.Structured | GraphicsBuffer.Target.Raw;
        ...
        public GeometryPool(in GeometryPoolDesc desc, ComputeShader geometryPoolShader, ComputeShader copyShader)
        {
            ...
            m_GlobalVertexBuffer = new GraphicsBuffer(ComputeBufferTarget | GraphicsBuffer.Target.Vertex, DivUp(m_MaxVertCounts * GetVertexByteSize(), 4), 4);
            m_GlobalIndexBuffer = new GraphicsBuffer(ComputeBufferTarget | GraphicsBuffer.Target.Index, m_MaxIndexCounts, 4);
            m_GlobalMeshChunkTableEntryBuffer = new GraphicsBuffer(ComputeBufferTarget, m_MaxMeshChunkTableEntriesCount, GetMeshChunkTableEntryByteSize());
            m_DummyBuffer = new GraphicsBuffer(ComputeBufferTarget, 16, 4);
        ...
        private bool AllocateGeo(Mesh mesh, out int allocationHandle)
        {
            ...
            if (mesh.subMeshCount > 0)
            {
                ...
                if (!newSlot.meshChunkTableAlloc.valid)
                {
                    ...
                    var newChunkTableBuffer = new GraphicsBuffer(ComputeBufferTarget, newChunkCount, GetMeshChunkTableEntryByteSize());
                ...
                for (int submeshIndex = 0; submeshIndex < mesh.subMeshCount; ++submeshIndex)
                {
                    ...
                    if (!newMeshChunk.vertexAlloc.valid)
                    {
                        ...
                        var newVertexBuffer = new GraphicsBuffer(ComputeBufferTarget | GraphicsBuffer.Target.Vertex, DivUp(newVertexCount * GetVertexByteSize(), 4), 4);
                    ...
                    if (!newMeshChunk.indexAlloc.valid)
                    {
                        ...
                        var newIndexBuffer = new GraphicsBuffer(ComputeBufferTarget | GraphicsBuffer.Target.Index, newIndexCount, 4);
